export const getMinDistance = (instructions1, instructions2) => {
  const line1 = parseLine(instructions1);
  const line2 = parseLine(instructions2);
  const intersections = getIntersections(line1, line2);
  const result = getClosestIntersection(intersections);
  return getIntersectionDistance(result);
};

export const getIntersectionDistance = intersection => {
  return Math.abs(intersection.x) + Math.abs(intersection.y);
};

export const getClosestIntersection = intersections => {
  let result = null;
  intersections
    ...
    .forEach(intersection => {
      if (!result) {
        ...
        getIntersectionDistance(intersection) < getIntersectionDistance(result) &&
        intersection.x !== 0 &&
        intersection.y !== 0
      ) {
        result = intersection;
...
export const getIntersections = (line1, line2) => {
  const results = [];
  for (let i = 1; i < line1.length; i++) {
    for (let j = 1; j < line2.length; j++) {
      let intersection = { x: 0, y: 0 };
      if (tryGetIntersectionPoint(line1[i - 1], line1[i], line2[j - 1], line2[j], intersection)) {
...
export const tryGetIntersectionPoint = (p1, p2, p3, p4, resultPoint) => {
  if (isHorizontal(p1, p2) && isVertical(p3, p4)) {
    if (isBetween(p1.y, p3.y, p4.y) && isBetween(p3.x, p1.x, p2.x)) {
      resultPoint.x = p3.x;
      resultPoint.y = p1.y;
      return true;
    }
  ...
  if (isVertical(p1, p2) && isHorizontal(p3, p4)) {
    if (isBetween(p3.y, p1.y, p2.y) && isBetween(p1.x, p3.x, p4.x)) {
      resultPoint.x = p1.x;
      resultPoint.y = p3.y;
      return true;
    }
...
export const isBetween = (target, p1, p2) => {
  return (target >= p1 && target <= p2) || (target >= p2 && target <= p1);
};

export const isHorizontal = (p1, p2) => {
  return p1.y === p2.y;
};

export const isVertical = (p1, p2) => {
  return p1.x === p2.x;
};

export const parseLine = instructions => {
  return parseInstructions(instructions).reduce((acc, currentInstruction, i) => {
    if (i === 0) {
      acc.push({ x: 0, y: 0 });
    }
    acc.push(calculateEndPoint(acc[i], currentInstruction));
    return acc;
  }, []);
};
...
export const calculateEndPoint = (startingPoint, instruction) => {
...
export const getMinimalSignalDelay = (instructions1, instructions2) => {
  const line1 = parseLine(instructions1);
  const line2 = parseLine(instructions2);
  const intersections = getIntersections(line1, line2);
  let minimumSum = -1;
  intersections
    .filter(i => i.x !== 0 && i.y !== 0)
    .forEach(intersection => {
      const stepsForLine1 = getStepsToPoint(line1, intersection);
      const stepsForLine2 = getStepsToPoint(line2, intersection);
      const sum = stepsForLine1 + stepsForLine2;
      if (minimumSum === -1) {
        minimumSum = sum;
      } else {
...
export const getStepsToPoint = (line, point) => {
  let steps = 0;
  for (let i = 1; i < line.length; i++) {
    ...
    let dX, dY;
    if (tryGetIntersectionPoint(previousPoint, currentPoint, point, point, resultPoint)) {
      dX = Math.abs(resultPoint.x - previousPoint.x);
      dY = Math.abs(resultPoint.y - previousPoint.y);
      steps += Math.max(dX, dY);
      return steps;
    } else {
      dX = Math.abs(currentPoint.x - previousPoint.x);
      dY = Math.abs(currentPoint.y - previousPoint.y);
      steps += Math.max(dX, dY);
    }
  }
  return steps;
...
console.log(`result for part 1 is ${getMinDistance(instructions1, instructions2)}`);
console.log(`result for part 2 is ${getMinimalSignalDelay(instructions1, instructions2)}`);
